...
class FeaturesExtractor(nn.Module):
    def __init__(self, config):
        ...
        base_kernel = config.down_kernel
        if isinstance(base_kernel, int):
            base_kernel = (base_kernel, base_kernel)
        kernel_h = max(1, min(base_kernel[0], out_h))
        kernel_w = max(1, min(base_kernel[1], out_w))
